...
- **Embedded assets.** Images and embedded objects render as their alt text in the Markdown, and the raw bytes stay available on the document model, tagged with their media type. Linked images render as their alt text too; the Markdown never carries an image URL.
...
- **Author-hidden content is omitted.** Word hidden ("vanish") text and hidden revision-deleted text in `.doc`/`.docx`/`.rtf`, hidden slides and hidden shapes in presentations, EPUB text styled `display:none` / `visibility:hidden` / `opacity:0` / `font-size:0` or carrying the `hidden` attribute or `aria-hidden="true"`, ODF `text:display` none and collapsed table rows, hidden rows, columns and sheets in Excel files and hidden rows in ODS, and tracked deletions and comments are all dropped. White-on-white text and off-page positioning are not detected. EPUB stylesheet rules apply when the selector is a tag, `.class`, `#id`, `[hidden]`, `[aria-hidden="true"]`, or a compound of those; rules with combinators, pseudo-classes, other attribute selectors, or escaped identifiers are ignored. A link whose label was entirely hidden is dropped with its destination; a link whose label is empty in the source still shows its URL.
